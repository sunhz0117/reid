...
class embed_net(nn.Module):
    def __init__(self,  class_num, attr_num=0, no_local='on', gm_pool='on', arch='resnet50'):
        ...
        self.classifier = nn.Linear(pool_dim, self.id_num, bias=False)
        if self.attr_num > 0:
            self.attr = nn.Linear(pool_dim, self.attr_num, bias=False)
    ...
    def forward(self, x1, x2, modal=0):
        ...
        if self.training:
            if self.attr_num > 0:
                return x_pool, self.classifier(feat), self.attr(feat)
            else:
