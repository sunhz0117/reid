...
parser.add_argument('--model_path', default='checkpoint/demo', type=str,
...
if dataset == 'sysu':
    ...
    print("Dataset statistics: IR --> RGB")
    print("  ------------------------------")
    ...
    for trial in range(args.trial):
        gall_img, gall_label, gall_cam = process_gallery_sysu(data_path, mode=args.mode, img_mode="rgb", trial=trial)
    ...
    cmc = all_cmc / args.trial
    mAP = all_mAP / args.trial

    cmc_pool = all_cmc_pool / args.trial
    mAP_pool = all_mAP_pool / args.trial
    print('All Average:')
    ...
    # draw CMC curve
    plt.figure()
    x = np.arange(0, len(cmc_pool))
    plt.plot(x, cmc_pool, color="red", linewidth=2, label="CMC")
    plt.xlabel("Rank")
    plt.ylabel("Matching Rate")
    plt.legend()
    plt.title("CMC Curve")
    plt.savefig(os.path.join(checkpoint_path, "CMC_Curve_IR2RGB.png"))


    # RGB --> IR

    # testing set
    query_img, query_label, query_cam = process_query_sysu(data_path, mode=args.mode, img_mode="rgb")
    gall_img, gall_label, gall_cam = process_gallery_sysu(data_path, mode=args.mode, img_mode="ir", trial=0)

    nquery = len(query_label)
    ngall = len(gall_label)
    print("Dataset statistics: RGB --> IR")
    print("  ------------------------------")
    print("  subset   | # ids | # images")
    print("  ------------------------------")
    print("  query    | {:5d} | {:8d}".format(len(np.unique(query_label)), nquery))
    ...
    query_feat_pool, query_feat_fc = extract_query_feat(query_loader)
    for trial in range(args.trial):
        gall_img, gall_label, gall_cam = process_gallery_sysu(data_path, mode=args.mode, img_mode="ir", trial=trial)
    ...
    plt.legend()
    plt.title("CMC Curve")
    plt.savefig(os.path.join(checkpoint_path, "CMC_Curve_RGB2IR.png"))
    ...
    for trial in range(10):
        test_trial = trial +1
        model_path = os.path.join(checkpoint_path, 'regdb_agw_p4_n8_lr_0.1_seed_0_trial_{}_best.t'.format(test_trial))
        if os.path.isfile(model_path):
            print('==> loading checkpoint {}'.format(args.resume))
            checkpoint = torch.load(model_path)
            net.load_state_dict(checkpoint['net'])
