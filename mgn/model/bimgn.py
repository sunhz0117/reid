...
from model.non_local import Non_local
from IPython import embed

def make_model(args):
    return BIMGN(args)

class BIMGN(nn.Module):
    def __init__(self, args):
        ...
        if args.non_local == True:
            ...
            self.backbone = nn.Sequential(
                resnet.layer1,
                resnet.layer2[0], resnet.layer2[1], resnet.layer2[2], self.NL_2[0], resnet.layer2[3], self.NL_2[1],
                resnet.layer3[0],
            )
            res_conv4 = nn.Sequential(resnet.layer3[1], resnet.layer3[2], resnet.layer3[3], self.NL_3[0], resnet.layer3[4], self.NL_3[1], resnet.layer3[5], self.NL_3[2])
    ...
    def forward(self, rgb_img, ir_img):
        ...
        # evaluate
        if ir_img is None:
            x = self.rgb_branch(rgb_img)
        elif rgb_img is None:
            x = self.ir_branch(ir_img)
        # train
        else:
            rgb_img = self.rgb_branch(rgb_img)
            ir_img = self.ir_branch(ir_img)
            x = torch.cat((rgb_img, ir_img), 0)
        ...
        # 39 attr CE LOSS
        attr_p1 = self.fc_attr_2048_0(fg_p1)
        attr_p2 = self.fc_attr_2048_1(fg_p2)
        attr_p3 = self.fc_attr_2048_2(fg_p2)
        ...
        return predict, fg_p1, fg_p2, fg_p3, l_p1, l_p2, l_p3, l0_p2, l1_p2, l0_p3, l1_p3, l2_p3, attr_p1, attr_p2, attr_p3
